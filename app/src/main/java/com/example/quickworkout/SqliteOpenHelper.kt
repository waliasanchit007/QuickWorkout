...
class SqliteOpenHelper(context: Context, factory : SQLiteDatabase.CursorFactory?) : SQLiteOpenHelper(context, DATABASE_NAME, factory, DATABASE_VERSION) {
    companion object{
        ...
        private val DATABASE_NAME = "QuickWorkout.db"
        private val TABLE_HISTORY = "history"
        private val COLUMN_ID = "_id"
        private val COMPLETED_DATE = "completed_date"
    }
    override fun onCreate(db: SQLiteDatabase?) {
        val CREATE_EXERCISE_TABLE = ("CREATE TABLE "+ TABLE_HISTORY +
                "(" + COLUMN_ID + " INTEGER PRIMARY KEY," + COMPLETED_DATE + " TEXT)")
    ...
    fun getAllCompletedDateList() :ArrayList<String>{
        ...
        while(cursor.moveToNext()){
            val dateValue = cursor.getString(cursor.getColumnIndex(COMPLETED_DATE))
            list.add(dateValue)
        }
        cursor.close()
        return list
